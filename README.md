...
| `writegate` | advisory, opt-in | Flags `github.com/moovfinancial/go-libs/observability/sql` writes (`Exec`, `ExecContext`, `ExecContextRetryable`, and methods that reach them) from HTTP handlers (`http.ResponseWriter`). HTTP should produce an event; consumers in both active regions apply the write because events are replicated. Writes are allowed in events consumer handlers (`eventing.EventHandlerContext`, `RecordHandler`, `EventMessageHandler`, and matching `func(context.Context, *events.Event) error` signatures). Detection is by type, not file name. One writing implementer marks the whole interface method. Enable with `writegate.enabled`. |
